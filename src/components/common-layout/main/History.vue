    ...
    <el-space :size="0" :spacer>
      <div v-for="item in histories" :key="item.path" class="p-1">
        <el-dropdown trigger="contextmenu">
          <span
            :class="['item', { active: item.path === route.fullPath }]"
            @click="tabClick(item)"
          >
...
const tabClick = (item: History) => router.push(item.path)
...
const remove = (item: History) => {
  ...
  const currentPage = item.path === route.fullPath
  if (currentPage) {
...
watch(
  () => route.path,
  () => {
    const exist = histories.value.find(v => v.path === route.fullPath)
    if (exist) return
