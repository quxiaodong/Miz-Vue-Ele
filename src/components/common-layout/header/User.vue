    ...
    <template #dropdown>
      <el-dropdown-menu>
        <el-dropdown-item command="password" icon="Lock">
          {{ translate({ stem: 'common:layout.change-password' }) }}
        </el-dropdown-item>
        <el-dropdown-item command="sign-out" icon="SwitchButton" divided>
          {{ translate({ stem: 'common:layout.sign-out' }) }}
        </el-dropdown-item>
      </el-dropdown-menu>
    </template>
  </el-dropdown>
  ...
  switch (type) {
    case 'password': {
      const dialog = createDialog(<Password onClose={() => dialog.close()} />, {
        title: translate({ stem: 'common:layout.change-password' })
      })
      break
    }
    case 'sign-out': {
      execute().then(() => {
        ElNotification.success(
          translate({ stem: 'common:layout.sign-out-success' })
        )
        removeToken()
        router.push('/sign-in')
      })
