<template>
  ...
  >
    ...
    <template v-if="isTag(schema) && schema.tag === 'el-input'">
      <el-input
        :placeholder="translate({ stem: 'common.please-input' })"
        v-bind="Object.assign(vModel(), schema.tagProps)"
      />
    </template>
    <template v-if="isTag(schema) && schema.tag === 'el-select'">
      <el-select
        :placeholder="translate({ stem: 'common.please-select' })"
        v-bind="Object.assign(vModel(), schema.tagProps)"
      >
        <el-option
    ...
    if (isTag(schema) && schema.tag === 'el-select') {
      if (schema.options) {
        ...
        if (schema.fetchOptions) {
          ...
          options.value = [
            { value: 0, label: translate({ stem: 'common.enable' }) },
            { value: 1, label: translate({ stem: 'common.disable' }) }
          ]
        }
      }
    }
